...
---
title: OpenClaw Cron Jobs — Canonical Reference
description: Atualize este arquivo a cada 7 dias com base em https://docs.openclaw.ai/automation/cron-jobs#cron-jobs. Este é o mastermind para criação e padronização de cron jobs no projeto.
---

# OpenClaw Cron Jobs — Canonical Reference

> Atualize este arquivo semanalmente a partir de https://docs.openclaw.ai/automation/cron-jobs#cron-jobs. Use **apenas este arquivo** como fonte para criar, revisar e padronizar todos os cron jobs do projeto.

## TL;DR
- Cron jobs são agendadores persistentes do Gateway OpenClaw.
- Dois estilos: main (system event, silencioso) e isolated (agent turn, com entrega/announce).
- Jobs são persistidos em ~/.openclaw/cron/jobs.json.
- Use sempre TELEGRAM_CHANNEL_ID para jobs Telegram.
- CLI principal: `openclaw cron add ...` (veja exemplos abaixo).

## Exemplos de Comando

### One-shot (main session, evento único)
```sh
openclaw cron add \
  --name "Reminder" \
...
### Recorrente isolado (entrega Telegram usuário)
```sh
openclaw cron add \
  --name "Morning brief" \
  --cron "0 7 * * *" \
  ...
  --announce \
  --channel telegram \
  --to "user:${TELEGRAM_CHANNEL_ID}"
```

### Recorrente isolado (entrega Telegram canal ou tópico)
```sh
openclaw cron add \
...
- Use sempre TELEGRAM_CHANNEL_ID para jobs Telegram (usuário ou canal).
- Use variáveis .env para campos sensíveis/configuráveis.
- O arquivo .cron.md deve conter **apenas o comando** (sem comentários ou metadados).
- Todos os jobs devem ser criados/revisados a partir deste mastermind.

## Referências
- [Documentação oficial OpenClaw Cron Jobs](https://docs.openclaw.ai/automation/cron-jobs#cron-jobs)
- [CLI: openclaw cron add](https://docs.openclaw.ai/cli/cron)
...
Main-session cron jobs create background task records with silent notify policy (no notifications by default). They appear in openclaw tasks list but do not generate delivery messages.
​
...
These detached runs create background task records visible in openclaw tasks and subject to task audit and maintenance.
​
...
One-shot (at) jobs retry transient errors (rate limit, overloaded, network, server_error) up to 3 times with backoff; permanent errors disable immediately. See Retry policy.
​
